...
function download_cat_taxonomy() {  
...
add_action( 'init', 'download_cat_taxonomy');
//Adds the download meta box for the download post type
function Template_meta_box_download() {
  add_meta_box( 'wp_custom_attachment', 'Download Settings', 'Template_meta_box_download_display', 'Template_download', 'normal', 'high' );
}
...
function Template_meta_box_download_display( $post, $box ) {
  ...
  // Add more!
   ...
   // Get the post meta
  $download = get_post_meta( $post->ID, 'wp_custom_attachment', true );
...
<table class="form-table">
   ...
   <tr>
      ...
      <td>
         <input type="text" id="download-file" size="60" name="download-file" value="<?php echo $value; ?>" />
         <input type="button" id="upload-file-button"  class="button" value="Upload file" onclick="document.getElementById('wp_custom_attachment').click()"/>
 ...
 */
   update_post_meta( $post_id, 'wp_custom_attachment', array( 
  'file' => sanitize_text_field($_POST['wp_custom_attachment']),
  ...
  update_post_meta( $post_id, 'wp_custom_attachment["file"]', sanitize_text_field( $_POST($upload['url'])));
  }
